...
export const BookExplorePage: React.FC<BookExplorePageProps> = ({ showAnnouncement = true }) => {
  const { books, addToCart, setActivePage, subsidyLedger, calculateBookSubsidy, setSelectedBookForDetail } = useShop();
    ...
    <div className='w-full bg-[#f6f6f6] min-h-screen py-8'>
      <div className='max-w-[1280px] mx-auto px-4 space-y-6'>
          ...
          <div className='bg-gradient-to-r from-[#181718] to-[#363636] text-white p-6 rounded-xl shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4'>
            <div className='space-y-1.5'>
              <div className='flex items-center gap-2'>
                <span className='bg-[#df0000] text-white text-xs font-bold px-2.5 py-0.5 rounded'>B2B 기업 독서지원</span>
                <span className='text-xs text-[#cbd2d4]'>임직원 복합결제 프로그램</span>
              </div>
              <h1 className='text-xl sm:text-2xl font-bold tracking-tight'>2026년 9월 독서 지원 프로그램 안내</h1>
              <p className='text-xs sm:text-sm text-[#cbd2d4] leading-relaxed'>
                • <strong>추천도서</strong>: 회사 지원금 100% 전액 지원 (월 1권, 직원 실결제 0원)
                <br />• <strong>개인도서</strong>: 도서가격의 50% 지원 (월 1권, 최대 10,000원 한도, 초과분 직원 결제)
              </p>
            </div>
            ...
            <div className='bg-white/10 backdrop-blur-sm border border-white/20 p-4 rounded-lg min-w-[280px] space-y-2 text-xs'>
              ...
              <div className='flex justify-between pt-1 text-[11px]'>
                <span className={subsidyLedger.recommendedUsed ? 'text-neutral-400 line-through' : 'text-[#a3e635] font-semibold'}>{subsidyLedger.recommendedUsed ? '✓ 추천도서 사용완료' : '● 추천도서 잔여 1권'}</span>
                <span className={subsidyLedger.personalUsed ? 'text-neutral-400 line-through' : 'text-[#38bdf8] font-semibold'}>{subsidyLedger.personalUsed ? '✓ 개인도서 사용완료' : '● 개인도서 잔여 1권'}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className='flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-lg border border-[#cbd2d4]'>
          <div className='flex items-center gap-2'>
            <button onClick={() => setFilterType('all')} className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${filterType === 'all' ? 'bg-[#181718] text-white shadow-sm' : 'bg-[#f6f6f6] text-[#595959] hover:bg-[#edf0f1]'}`}>
              전체 도서 ({books.length})
            </button>
            <button onClick={() => setFilterType('recommended')} className={`px-4 py-2 rounded-full text-sm font-semibold transition-all flex items-center gap-1.5 ${filterType === 'recommended' ? 'bg-[#df0000] text-white shadow-sm' : 'bg-[#f6f6f6] text-[#595959] hover:bg-[#edf0f1]'}`}>
              <Award className='w-4 h-4' />
              추천도서 (회사 100% 지원)
            </button>
            <button onClick={() => setFilterType('personal')} className={`px-4 py-2 rounded-full text-sm font-semibold transition-all flex items-center gap-1.5 ${filterType === 'personal' ? 'bg-[#1f976b] text-white shadow-sm' : 'bg-[#f6f6f6] text-[#595959] hover:bg-[#edf0f1]'}`}>
              <Sparkles className='w-4 h-4' />
              개인도서 (50% 복합지원)
            </button>
          </div>

          <div className='text-xs text-[#80888a]'>* 도서를 선택하시면 실시간 지원금 계산 결과를 확인하실 수 있습니다.</div>
        </div>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
          {filteredBooks.map((book) => {
            ...
            return (
              <div key={book.id} className='bg-white rounded-lg border border-[#cbd2d4] hover:border-[#80888a] hover:shadow-md transition-all flex flex-col justify-between overflow-hidden'>
                <div className='p-4 space-y-3'>
                  {/* Badge & Type */}
                  <div className='flex items-center justify-between text-xs'>
                    <span className={`px-2 py-0.5 rounded font-bold ${isRecommended ? 'bg-[#ffebeb] text-[#df0000] border border-[#f9cdcd]' : 'bg-[#edf0f1] text-[#1f976b] border border-[#cbd2d4]'}`}>{isRecommended ? '추천도서 100% 지원' : '개인도서 50% 지원'}</span>
                  ...
                  {/* Title & Author */}
                  <div>
                    <h3 onClick={() => setSelectedBookForDetail(book)} className='font-bold text-[#181718] text-base hover:text-[#df0000] cursor-pointer line-clamp-1'>
                      {book.title}
                    </h3>
                  ...
                  <div className='flex items-center gap-2 pt-1'>
                    <button type='button' onClick={() => handleFormatChange(book.id, 'paper')} className={`flex-1 py-1 rounded text-xs font-medium border ${format === 'paper' ? 'bg-[#181718] text-white border-[#181718]' : 'bg-white text-[#595959] border-[#cbd2d4] hover:bg-[#f6f6f6]'}`}>
                      종이책
                    </button>
                    <button type='button' onClick={() => handleFormatChange(book.id, 'ebook')} className={`flex-1 py-1 rounded text-xs font-medium border ${format === 'ebook' ? 'bg-[#181718] text-white border-[#181718]' : 'bg-white text-[#595959] border-[#cbd2d4] hover:bg-[#f6f6f6]'}`}>
                      전자책(eBook)
                    </button>
                  </div>
                  ...
                  <div className='bg-[#fffafa] border border-[#f9cdcd] rounded p-2.5 text-xs space-y-1'>
                    ...
                    {isRecommended && format === 'ebook' && <p className='text-[10px] text-[#df0000] pt-1 font-semibold'>⚠️ 추천도서는 종이도서만 지원 가능 (전자책 지원불가)</p>}
                  </div>
                </div>

                {/* Card Action Buttons */}
                <div className='p-4 pt-0 grid grid-cols-2 gap-2'>
                  <button onClick={() => addToCart(book, format, 1, false)} className='py-2.5 px-3 rounded border border-[#cbd2d4] bg-white hover:bg-[#f6f6f6] text-xs font-bold text-[#181718] flex items-center justify-center gap-1.5 transition-colors'>
                    <ShoppingCart className='w-3.5 h-3.5' />
                    장바구니
                  </button>
                  <button onClick={() => addToCart(book, format, 1, true)} className='py-2.5 px-3 rounded bg-[#df0000] hover:bg-[#ea2e2e] text-xs font-bold text-white flex items-center justify-center gap-1.5 transition-colors shadow-sm'>
          ...
          <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4'>
            <div className='bg-white w-full max-w-xl rounded-xl shadow-2xl overflow-hidden border border-[#cbd2d4] animate-in fade-in duration-150'>
              <div className='bg-[#181718] text-white px-5 py-4 flex items-center justify-between'>
                ...
                <button onClick={() => setActiveQuickDetailBook(null)} className='text-neutral-400 hover:text-white'>
                  ✕
                </button>
              </div>
              ...
              <div className='p-6 space-y-4'>
                ...
                <div className='text-xs text-[#555a5c] bg-[#f6f6f6] p-3 rounded leading-relaxed'>{activeQuickDetailBook.description}</div>

                {/* Subsidies Calculation Table */}
                <div className='bg-[#fffafa] border border-[#f9cdcd] p-4 rounded-lg space-y-2 text-sm'>
                  <div className='font-bold text-[#181718] text-xs pb-1 border-b border-[#f5baba]'>B2B 복합결제 적용 내역 (임직원 독서지원 규정)</div>
