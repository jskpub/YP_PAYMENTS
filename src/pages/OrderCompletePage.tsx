  ...
  if (!order) {
    return (
      <div className='max-w-[1280px] mx-auto px-4 py-16 text-center space-y-4'>
        <h2 className='text-h3 font-bold text-[#181718]'>주문 내역이 없습니다.</h2>
        <button onClick={() => setActivePage('explore')} className='px-6 py-2.5 bg-[#df0000] text-white rounded text-body-xs font-bold'>
          도서 둘러보기
        </button>
      </div>
    );
      ...
      <div className='max-w-[840px] mx-auto px-4 space-y-6'>
        ...
        <div className='bg-white rounded-xl border border-[#cbd2d4] p-8 text-center space-y-4 shadow-sm'>
          ...
          <div className='space-y-1'>
            <span className='text-caption text-[#1f976b] font-bold bg-[#e8f5ef] px-3 py-1 rounded-full inline-block'>B2B 복합결제 정상 승인 완료</span>
            <h1 className='text-h2 sm:text-h1 font-bold text-[#181718] tracking-tight'>주문이 성공적으로 완료되었습니다!</h1>
            <p className='text-body-xs text-[#80888a]'>주문하신 상품의 배송 준비가 시작되며, 알림톡으로 배송 정보를 안내해 드립니다.</p>
          </div>

          <div className='bg-[#f6f6f6] rounded-lg p-4 max-w-md mx-auto space-y-1 text-caption'>
            <div className='flex justify-between items-center'>
              <span className='text-[#80888a]'>통합 주문번호</span>
              <span className='flex items-center gap-0.5'>
                <span className='font-bold text-body-xs text-[#181718]'>{order.orderId}</span>
                {/* 개선 항목 1: 주문번호 클립보드 복사 */}
                <button
                  type='button'
        ...
        <div className='bg-white rounded-xl border border-[#cbd2d4] p-6 space-y-4 shadow-sm'>
          ...
          <div className='flex flex-col sm:flex-row items-center gap-2 sm:gap-3'>
            <div className='w-full sm:flex-1 bg-[#f6f6f6] p-4 rounded-lg text-center' aria-label={`총 주문금액 ${order.totalSellingPrice.toLocaleString()}원`}>
              <span className='text-caption text-[#80888a]'>총 도서 금액</span>
          ...
          <div className='text-caption bg-[#f6f6f6] p-3 rounded space-y-1 text-[#555a5c]'>
            <div className='flex justify-between'>
              <span>직원 결제수단</span>
        ...
        <div className='bg-white rounded-xl border border-[#cbd2d4] overflow-hidden shadow-sm'>
          <div className='p-5 border-b border-[#dadada]'>
            <h2 className='text-body-md font-bold text-[#181718]'>주문 상품 정보 ({order.items.length}종)</h2>
          </div>
        ...
        <div className='bg-white rounded-xl border border-[#cbd2d4] p-6 space-y-2 shadow-sm text-caption sm:text-body-xs'>
          <h2 className='text-body-md font-bold text-[#181718] border-b border-[#dadada] pb-3 flex items-center gap-2'>
            <Truck className='w-4 h-4 text-[#df0000]' />
            배송지 정보
          </h2>
          <div className='grid grid-cols-[100px_1fr] gap-2 pt-1 text-caption'>
