        ...
        <div>
          <div className="flex items-center justify-between border-b border-slate-100 p-6 pb-4">
            ...
            <div className="grid grid-cols-3 gap-0.5 opacity-30">
              {Array.from({ length: 6 }).map((_, index) => <span key={index} className="h-1 w-1 rounded-full bg-slate-700" />)}
            </div>
          </div>
          ...
          <nav className="space-y-1.5 p-4 text-xs">
            {portalMenus.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex w-full items-center gap-2.5 rounded-xl px-3.5 py-2.5 font-medium transition-colors ${
                  activeMenu === id ? 'bg-[#1428a0] font-semibold text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
            ...
            <div className="pb-2 pt-4">
              ...
              <button
                ...
                className="group flex w-full items-center justify-between rounded-xl border border-red-200 bg-gradient-to-r from-red-50 to-rose-50/60 p-3 text-left transition-all hover:border-[#df0000] hover:shadow-sm"
              >
                <span className="flex items-center gap-2.5">
                  ...
                  <span>
                    <span className="flex items-center gap-1 text-xs font-bold text-slate-900 group-hover:text-[#df0000]">영풍문고 비즈몰 <b className="rounded bg-[#df0000] px-1 text-[9px] text-white">B2B</b></span>
          ...
          <div className="flex items-center justify-between rounded-2xl bg-[#1428a0] px-5 py-3.5 text-white shadow-sm sm:px-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden items-center gap-3 border-l border-white/20 pl-3 sm:flex">
                ...
                <div className="flex h-8 w-8 items-center justify-center rounded-full border border-white/30 bg-white/20"><Users className="h-4 w-4" /></div>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 space-y-4 overflow-y-auto p-4">
          <section className="space-y-4 rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="flex items-center gap-4">
                <div className="flex rounded-lg bg-slate-100 p-0.5 text-xs font-medium text-slate-600">
                  {(['일', '주', '월'] as const).map((view) => (
                    <button key={view} type="button" onClick={() => setCalendarView(view)} className={`rounded-md px-3 py-1 ${calendarView === view ? 'bg-slate-800 font-semibold text-white shadow-sm' : 'hover:text-slate-900'}`}>{view}</button>
                  ))}
                </div>
                <div className="hidden items-center gap-3 text-xs font-medium text-slate-600 md:flex">
                  <span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#1428a0]" />정상근무</span>
                  <span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-emerald-500" />조퇴</span>
                  <span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-amber-400" />연차</span>
                </div>
              </div>
            </div>
            <div className="overflow-hidden rounded-xl border border-slate-200">
              <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50/70 p-2.5 text-xs font-medium text-slate-600">
                <button type="button" className="rounded p-1 text-slate-500 hover:bg-slate-200" aria-label="이전 주">‹</button>
                <span>2026.09.14 ~ 2026.09.20</span>
                <button type="button" className="rounded p-1 text-slate-500 hover:bg-slate-200" aria-label="다음 주">›</button>
              </div>
          ...
          <section className="grid grid-cols-1 gap-4 xl:grid-cols-3">
            <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm xl:col-span-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3"><h2 className="font-bold text-slate-900">오늘의 할 일</h2><span className="text-xs text-slate-400">2026.09.16</span></div>
              <div className="grid grid-cols-2 gap-3 pt-4 sm:grid-cols-4">
                {['결재 대기', '공지사항', '교육 현황', '인사 알림'].map((label, index) => <div key={label} className="rounded-xl bg-slate-50 p-4"><span className="text-xs text-slate-500">{label}</span><strong className="mt-2 block text-xl text-slate-900">{index === 0 ? '3' : index === 1 ? '2' : '0'}</strong></div>)}
              </div>
            </div>
            <div className="rounded-2xl border border-red-100 bg-gradient-to-br from-red-50 to-white p-5 shadow-sm">
              ...
              <button type="button" onClick={() => setActivePage('recommended')} className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-[#df0000] px-3.5 py-2 text-xs font-bold text-white hover:bg-[#c90000]">비즈몰 바로가기 <ArrowUpRight className="h-3.5 w-3.5" /></button>
            </div>
          </section>
        </main>
      </div>
