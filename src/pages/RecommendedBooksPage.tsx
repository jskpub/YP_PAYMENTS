...
export const RecommendedBooksPage: React.FC = () => {
  ...
  const recommendedBooks = books.filter((book) => book.id.startsWith('rec-'));
  ...
  return (
    <div className="w-full bg-white text-[#181718]">
      ...
      <main className="mx-auto w-full max-w-[1200px] px-4 py-12">
              ...
              <article key={book.id} className="flex flex-col rounded-2xl border border-[#cbd2d4] bg-white p-6 transition-shadow hover:shadow-md">
                <div className="flex gap-5">
                  <div className="relative shrink-0">
                    ...
                    <button type="button" onClick={() => setSelectedBookForDetail(book)}><img src={book.coverImage} alt={book.title} className="h-44 w-32 rounded object-cover bg-[#f6f6f6] shadow-lg" /></button>
                  </div>
                  <div className="flex min-w-0 flex-1 flex-col gap-2">
                    <span className="w-fit rounded bg-[#f6f6f6] px-2 py-0.5 text-[11px] font-bold text-[#595959]">임직원 추천도서</span>
                    <button type="button" onClick={() => setSelectedBookForDetail(book)} className="text-left text-lg font-bold leading-snug hover:text-[#df0000]">{book.title}</button>
                    <p className="text-sm text-[#80888a]">{book.author} · {book.publisher}</p>
                    <p className="text-xs text-[#9c9c9c]">{book.publishDate} · {book.tags[0]}</p>
                ...
                <div className="mt-5 min-h-32 rounded-xl bg-[#f6f6f6] p-4">
                  ...
                  <div className="mt-3 flex gap-2 border-t border-[#dadada] pt-3 text-xs text-[#80888a]"><Quote className="h-3.5 w-3.5 shrink-0 text-[#df0000]" />회사와 함께 읽는 이번 달의 큐레이션 도서입니다.</div>
                </div>

                <div className="mt-4 border-t border-[#edf0f1] pt-4 text-sm text-[#80888a]"><Heart className="mr-1 inline h-4 w-4 fill-current text-[#e4453a]" />이번 달 {book.reviewCount}명 관심</div>
                <div className="mt-4 grid grid-cols-2 gap-2">
                  {isBlocked ? (
                    ...
                    <>
                      <button type="button" onClick={() => inCart ? removeFromCart(cart.find((item) => item.book.id === book.id)!.id) : addToCart(book)} className={`flex h-12 items-center justify-center gap-1.5 rounded-lg text-sm font-bold ${inCart ? 'border-2 border-[#df0000] text-[#df0000]' : 'border border-[#c3c2c2] text-[#181718]'}`}>
                        {inCart ? <Check className="h-4 w-4" /> : <ShoppingCart className="h-4 w-4" />}
                        {inCart ? '담음 · 빼기' : '장바구니 담기'}
                      </button>
                      <button type="button" onClick={() => addToCart(book, 'paper', 1, true)} className="h-12 rounded-lg bg-[#df0000] text-sm font-bold text-white hover:bg-[#ea2e2e]">구매하기</button>
                    </>
                  )}
                </div>
              </article>
        ...
        <button type="button" onClick={() => setActivePage('home')} className="mx-auto mt-10 flex items-center gap-1 text-sm font-semibold text-[#595959] hover:text-[#df0000]"><ChevronRight className="h-4 w-4 rotate-180" />비즈몰 홈으로 돌아가기</button>
      </main>
    </div>
  );
};
