...
import { Award, BookOpen, Calendar, CheckCircle2, FileText, RotateCcw, Sparkles, Truck, XCircle, AlertCircle } from 'lucide-react';

export const MyPage: React.FC = () => {
  const { myPageTab, setMyPageTab, subsidyLedger, orders, cancelOrder, openReceiptModal, setActivePage } = useShop();
      ...
      <div className='max-w-[1280px] mx-auto px-4 space-y-6'>
        ...
        <div className='bg-white rounded-xl border border-[#cbd2d4] p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4'>
          <div className='flex items-center gap-4'>
            <div className='w-14 h-14 rounded-full bg-[#df0000] text-white flex items-center justify-center font-black text-xl shadow-xs'>김</div>
            <div>
              <div className='flex items-center gap-2'>
                <h1 className='text-xl font-bold text-[#181718]'>김민서 임직원님</h1>
                <span className='bg-[#e8f5ef] text-[#1f976b] text-xs font-bold px-2.5 py-0.5 rounded border border-[#a3d9bc]'>B2B 독서지원 대상자</span>
              </div>
              <p className='text-xs text-[#80888a] mt-0.5'>소속: (주)파트너스 B2B | 사번: EMP-2026-9243 | 회원등급: 골드회원</p>
            </div>
          </div>
        ...
        <div className='flex border-b border-[#dadada] bg-white rounded-t-lg px-4 pt-2 text-sm font-semibold gap-2'>
          <button onClick={() => setMyPageTab('subsidy')} className={`py-3 px-5 transition-colors border-b-2 ${myPageTab === 'subsidy' ? 'border-[#df0000] text-[#df0000] font-bold' : 'border-transparent text-[#80888a] hover:text-[#181718]'}`}>
            나의 B2B 독서지원금 (Rule & Ledger)
          </button>
          <button onClick={() => setMyPageTab('orders')} className={`py-3 px-5 transition-colors border-b-2 ${myPageTab === 'orders' ? 'border-[#df0000] text-[#df0000] font-bold' : 'border-transparent text-[#80888a] hover:text-[#181718]'}`}>
            주문 / 배송 내역 ({orders.length})
          </button>
          <button onClick={() => setMyPageTab('refund')} className={`py-3 px-5 transition-colors border-b-2 ${myPageTab === 'refund' ? 'border-[#df0000] text-[#df0000] font-bold' : 'border-transparent text-[#80888a] hover:text-[#181718]'}`}>
            취소 / 환불 정책 안내
          </button>
        </div>
          ...
          <div className='space-y-6'>
            ...
            <div className='bg-white rounded-xl border border-[#cbd2d4] p-6 shadow-sm space-y-4'>
              <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-[#edf0f1] pb-4'>
                <div>
                  ...
                  <p className='text-xs text-[#80888a] mt-0.5'>회사에서 임직원 역량 개발을 위해 제공하는 복지 지원 프로그램입니다.</p>
                </div>
                <div className='text-right'>
                  <span className='text-xs text-[#80888a]'>월 한도: </span>
                  <span className='text-sm font-bold text-[#181718]'>{subsidyLedger.monthlyLimit.toLocaleString()}원</span>
                </div>
              </div>
              ...
              <div className='space-y-2'>
                <div className='flex justify-between text-xs font-semibold'>
                  <span className='text-[#1f976b]'>총 사용 지원금: {subsidyLedger.totalUsedSubsidy.toLocaleString()}원</span>
                  <span className='text-[#df0000]'>잔여 한도: {subsidyLedger.remainingSubsidy.toLocaleString()}원</span>
                </div>
                <div className='w-full bg-[#f6f6f6] h-3.5 rounded-full overflow-hidden border border-[#edf0f1]'>
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.round((subsidyLedger.totalUsedSubsidy / subsidyLedger.monthlyLimit) * 100))}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4 pt-2'>
                {/* 추천도서 Rule Card */}
                <div className={`p-4 rounded-lg border transition-all ${subsidyLedger.recommendedUsed ? 'border-[#a3d9bc] bg-[#e8f5ef]/40' : 'border-[#cbd2d4] bg-white'}`}>
                  <div className='flex items-center justify-between'>
                    <span className='text-xs font-bold text-[#df0000] bg-[#ffebeb] px-2 py-0.5 rounded'>추천도서 Rule</span>
                    <span className={`text-xs font-semibold ${subsidyLedger.recommendedUsed ? 'text-[#1f976b]' : 'text-[#80888a]'}`}>{subsidyLedger.recommendedUsed ? '✓ 이달 사용 완료' : '● 신청 가능 (잔여 1권)'}</span>
                  </div>
                  <h3 className='font-bold text-base text-[#181718] mt-2'>회사 지원 100% 전액 지원</h3>
                  <p className='text-xs text-[#595959] mt-1 leading-relaxed'>월 1권에 한해 도서 금액 전액을 회사에서 지원합니다. (직원 실결제 0원)</p>
                  {!subsidyLedger.recommendedUsed && (
                    <button onClick={() => setActivePage('explore')} className='mt-3 text-xs font-bold text-[#df0000] hover:underline'>
                      추천도서 목록 보러가기 &gt;
                    </button>
                  )}
                </div>
                ...
                {/* 개인도서 Rule Card */}
                <div className={`p-4 rounded-lg border transition-all ${subsidyLedger.personalUsed ? 'border-[#a3d9bc] bg-[#e8f5ef]/40' : 'border-[#cbd2d4] bg-white'}`}>
                  <div className='flex items-center justify-between'>
                    <span className='text-xs font-bold text-[#1f976b] bg-[#e8f5ef] px-2 py-0.5 rounded'>개인도서 Rule</span>
                    <span className={`text-xs font-semibold ${subsidyLedger.personalUsed ? 'text-[#1f976b]' : 'text-[#80888a]'}`}>{subsidyLedger.personalUsed ? '✓ 이달 사용 완료' : '● 신청 가능 (잔여 1권)'}</span>
                  </div>
                  <h3 className='font-bold text-base text-[#181718] mt-2'>50% 지원 (최대 10,000원)</h3>
                  <p className='text-xs text-[#595959] mt-1 leading-relaxed'>월 1권에 한해 도서가격의 50%(최대 1만원)를 지원하며, 초과분은 직원이 결제합니다.</p>
                  {!subsidyLedger.personalUsed && (
                    <button onClick={() => setActivePage('explore')} className='mt-3 text-xs font-bold text-[#1f976b] hover:underline'>
                      개인도서 목록 보러가기 &gt;
                    </button>
                  )}
                </div>
            ...
            <div className='bg-white rounded-xl border border-[#cbd2d4] p-6 shadow-sm space-y-4'>
              ...
              {subsidyLedger.history.length === 0 ? (
                <div className='py-8 text-center text-xs text-[#80888a]'>아직 지원금 사용 내역이 없습니다.</div>
              ) : (
                      ...
                      {subsidyLedger.history.map((record, idx) => (
                        <tr key={idx} className='hover:bg-[#fafafa]'>
                          ...
                          <td className='p-3 text-center'>
                            <span className={`px-2 py-0.5 rounded font-bold ${record.bookType === 'recommended' ? 'bg-[#ffebeb] text-[#df0000]' : 'bg-[#e8f5ef] text-[#1f976b]'}`}>{record.bookType === 'recommended' ? '추천 100%' : '개인 50%'}</span>
                          </td>
                          <td className='p-3 text-right'>{record.sellingPrice.toLocaleString()}원</td>
                          <td className='p-3 text-right font-bold text-[#1f976b]'>- {record.companySubsidy.toLocaleString()}원</td>
                          <td className='p-3 text-right font-bold text-[#df0000]'>{record.employeePayment.toLocaleString()}원</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className='space-y-4'>
            {orders.length === 0 ? (
              <div className='bg-white rounded-xl border border-[#cbd2d4] p-16 text-center space-y-3'>
                ...
                <button onClick={() => setActivePage('explore')} className='px-6 py-2.5 bg-[#df0000] text-white rounded text-xs font-bold hover:bg-[#ea2e2e]'>
                  도서 둘러보기
                </button>
              </div>
            ) : (
              orders.map((order) => {
                ...
                return (
                  <div key={order.orderId} className='bg-white rounded-xl border border-[#cbd2d4] p-5 space-y-4 shadow-sm'>
                    {/* Order header row */}
                    <div className='flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#edf0f1] pb-3 gap-2'>
                      <div className='flex items-center gap-3'>
                        ...
                        <span className={`text-xs px-2.5 py-0.5 rounded font-bold ${isCancelled ? 'bg-[#edf0f1] text-[#9c9c9c] line-through' : 'bg-[#e8f5ef] text-[#1f976b]'}`}>
                          {order.orderStatus === 'completed' ? '결제완료' : order.orderStatus === 'preparing' ? '배송준비중' : '주문취소/환불완료'}
                        </span>
                      </div>

                      <div className='flex items-center gap-2'>
                        <button onClick={() => openReceiptModal(order)} className='px-3 py-1.5 rounded border border-[#cbd2d4] bg-white hover:bg-[#f6f6f6] text-xs font-semibold text-[#181718] flex items-center gap-1'>
                          <FileText className='w-3.5 h-3.5 text-[#df0000]' />
                          전자영수증
                        </button>
                        {!isCancelled && (
                          <button onClick={() => cancelOrder(order.orderId)} className='px-3 py-1.5 rounded border border-[#f9cdcd] bg-[#fffafa] hover:bg-[#ffebeb] text-xs font-semibold text-[#df0000] flex items-center gap-1'>
                    ...
                    <div className='divide-y divide-[#edf0f1]'>
                      {order.items.map((item, idx) => (
                        <div key={idx} className='py-2.5 flex items-center justify-between text-xs sm:text-sm'>
                          <div className='flex items-center gap-3'>
                            <img src={item.coverImage} alt={item.title} className='w-12 h-16 object-contain rounded border border-[#edf0f1]' />
                            <div>
                              <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${item.bookType === 'recommended' ? 'bg-[#ffebeb] text-[#df0000]' : 'bg-[#e8f5ef] text-[#1f976b]'}`}>{item.bookType === 'recommended' ? '추천 100% 지원' : '개인 50% 지원'}</span>
                    ...
                    <div className='bg-[#f6f6f6] rounded-lg p-3 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs gap-2'>
                      <div className='space-y-0.5'>
                        ...
                        <div className='text-[#80888a]'>결제수단: {order.paymentMethod}</div>
                      </div>
                      ...
                      <div className='text-right sm:text-right text-xs'>
                        ...
                        <div className='text-sm font-black text-[#df0000] mt-0.5'>직원 실결제: {order.finalPaidAmount.toLocaleString()}원</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        {myPageTab === 'refund' && (
          <div className='bg-white rounded-xl border border-[#cbd2d4] p-6 space-y-6 shadow-sm text-sm text-[#3d3c3f]'>
            <div>
              <h2 className='text-lg font-bold text-[#181718] mb-1'>B2B 복합결제 취소 및 환불 프로세스 안내</h2>
              <p className='text-xs text-[#80888a]'>영풍문고 B2B 독서지원 프로그램에 따른 주문 취소 및 환불 기준입니다.</p>
            </div>
