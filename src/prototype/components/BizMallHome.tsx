...
import { Bookmark, Calendar, ChevronRight, Gift, Info, MapPin, Settings, Star, Tag, Ticket, User } from 'lucide-react';
...
export const BizMallHome: React.FC<BizMallHomeProps> = ({ benefit, activeMenu, searchQuery, cart, onAddToCart, onRemoveFromCart, onOpenBook, onPendingNav }) => {
  const query = searchQuery.trim().toLowerCase();
  const cartIds = new Set(cart.map((line) => line.bookId));

  const books = MOCK_BOOKS.filter((book) => {
    const matchesMenu = activeMenu === 'ALL' || (activeMenu === 'RECOMMENDED' && book.supportType === 'recommended') || (activeMenu === 'BEST' && book.isBestseller) || (activeMenu === 'NEW' && book.isNewRelease);
  ...
  return (
    <div className='flex-1'>
      ...
      <section className='bg-yp-ink text-white'>
        <div className='mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 px-4 py-10 md:flex-row md:items-center'>
          ...
          <div className='w-full shrink-0 space-y-2 rounded-2xl border border-white/15 bg-white/10 p-5 md:w-80'>
            <div className='border-b border-white/15 pb-2 text-xs text-white/70'>{benefit.cycleLabel} 지원 현황</div>
            ...
            <div className='flex items-center justify-between py-1'>
              <span className='text-xs text-white/80'>개인 도서 ({benefit.personalBook.rate}% 지원)</span>
              <span className='text-sm font-bold'>{benefit.personalBook.remainingAmount.toLocaleString()}원 남음</span>
            </div>
            ...
            <div className='h-1.5 w-full overflow-hidden rounded-full bg-white/20'>
              <div
                ...
                style={{
                  width: `${(benefit.personalBook.remainingAmount / benefit.personalBook.limitAmount) * 100}%`,
                }}
              />
            </div>
        ...
        <div className='mx-auto max-w-7xl px-4'>
          <div className='grid grid-cols-4 gap-3 text-center sm:grid-cols-8'>
            {QUICK_MENUS.map(({ label, icon: Icon }) => (
              <button key={label} type='button' onClick={() => onPendingNav(label)} className='group flex cursor-pointer flex-col items-center gap-1.5 focus:outline-none'>
      ...
      <main className='mx-auto w-full max-w-7xl px-4 py-8'>
        ...
        {books.length === 0 ? (
          <p className='rounded-2xl border border-yp-gray-200 bg-white py-16 text-center text-sm text-yp-gray-500'>조건에 맞는 도서가 없습니다.</p>
        ) : (
          <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4'>
            {books.map((book) => {
              return (
                <article key={book.id} className='flex flex-col justify-between overflow-hidden rounded-2xl border border-yp-gray-200 bg-white transition-shadow hover:shadow-md'>
                  <div>
                    <div className='relative flex h-48 items-center justify-center bg-yp-gray-100 p-4'>
                      <button type='button' aria-label={`${book.title} 상세 보기`} onClick={() => onOpenBook(book.id)} className='cursor-pointer'>
                        <BookCover book={book} className='h-40 w-28 shadow-lg' />
                      </button>
                      {book.supportType === 'recommended' && <span className={`absolute left-2.5 top-2.5 rounded px-2 py-0.5 text-[10px] font-bold ${RECOMMENDED_BADGE.className}`}>{RECOMMENDED_BADGE.label}</span>}
                      {book.isNewRelease && <span className='absolute right-2.5 top-2.5 rounded bg-white/90 px-2 py-0.5 text-[10px] font-bold text-yp-ink'>신간</span>}
                    </div>
                    ...
                    <div className='space-y-2 p-4'>
                      ...
                      <button type='button' onClick={() => onOpenBook(book.id)} className='cursor-pointer text-left'>
                        <h3 className='line-clamp-2 text-sm font-bold leading-snug text-yp-ink hover:text-yp-red'>{book.title}</h3>
                      </button>
                  ...
                  <div className='grid grid-cols-2 gap-2 p-4 pt-0'>
                    <button type='button' onClick={() => onOpenBook(book.id)} className='cursor-pointer rounded-xl border border-yp-gray-200 px-2.5 py-2 text-xs font-semibold text-yp-gray-700 transition-colors hover:bg-yp-gray-50'>
                      상세 보기
                    </button>
                    {cartIds.has(book.id) ? (
                      <button type='button' onClick={() => onRemoveFromCart(book.id)} className='cursor-pointer rounded-xl border-2 border-yp-red px-2.5 py-2 text-xs font-bold text-yp-red transition-colors hover:bg-yp-red-soft'>
                        담음 · 빼기
                      </button>
                    ) : (
                      <button type='button' onClick={() => onAddToCart(book.id)} className='cursor-pointer rounded-xl bg-yp-red px-2.5 py-2 text-xs font-bold text-white transition-colors hover:bg-yp-red-hover'>
                        담기
                      </button>
                    )}
                  </div>
