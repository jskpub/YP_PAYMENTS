  ...
  return (
    ...
    >
          ...
          <div className="relative flex items-center gap-3">
            <div className="relative py-1" onMouseEnter={openUserMenu} onMouseLeave={scheduleCloseUserMenu}>
              <button
                ...
                className="flex cursor-pointer items-center gap-1.5 font-medium text-yp-ink hover:text-yp-red"
              >
                <span className="flex h-4 w-4 items-center justify-center rounded-full bg-emerald-600 text-[10px] font-bold leading-none text-white">
                  {employeeName.charAt(0)}
                </span>
                <span className="font-semibold">{employeeName}</span>
                <ChevronDown
                  size={12}
                  className={`text-yp-gray-400 transition-transform ${isUserMenuOpen ? 'rotate-180' : ''}`}
                />
              </button>
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 top-full z-50 w-72 pt-1">
                  <div className="overflow-hidden rounded-lg border border-yp-gray-200 bg-white text-xs shadow-xl">
                    <div className="flex items-center gap-3 p-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="rounded border border-emerald-600 px-1.5 text-[10px] font-bold leading-tight text-emerald-600">
                            임직원
                          </span>
                        </div>
                        <div className="flex items-center gap-1 text-[11px] text-yp-gray-500">
                          <button
                            type="button"
                            onClick={() => goPending('마이페이지')}
                            className="cursor-pointer font-medium hover:text-yp-red hover:underline"
                          >
                            마이페이지
                          </button>
                          <span>|</span>
                          <button
                            type="button"
                            onClick={() => goPending('회원정보 관리')}
                            className="cursor-pointer hover:text-yp-red hover:underline"
                          >
                            내 정보
                          </button>
                          <span>|</span>
                          <button
                            type="button"
                            onClick={onLogout}
                            className="cursor-pointer hover:text-yp-ink hover:underline"
                          >
                            로그아웃
                          </button>
                        </div>
                    ...
                    <div className="flex items-center justify-between border-y border-yp-red-soft bg-yp-red-soft px-4 py-2 text-[11px]">
                      <span className="font-bold text-yp-red">이번 달 남은 지원금</span>
                      <button
                        type="button"
                        onClick={() => goPending('나의 지원금')}
                        className="cursor-pointer font-bold text-yp-red hover:underline"
                      >
      ...
      <div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 px-4 py-4 md:flex-row md:items-center">
        ...
        <div className="mx-0 max-w-xl flex-1 md:mx-4">
          ...
          >
            ...
            <button
              type="submit"
              aria-label="도서 검색"
              className="absolute right-3.5 cursor-pointer rounded-full p-1 text-yp-gray-500 hover:text-yp-red"
            >
              <Search size={20} />
            </button>
          </form>
          <div className="scrollbar-none mt-1.5 flex items-center gap-2 overflow-x-auto whitespace-nowrap text-[11px] text-yp-gray-500">
        ...
        <button
          type="button"
          onClick={() => onPendingNav('나의 지원금')}
          className="flex shrink-0 cursor-pointer items-center gap-3 rounded-xl border border-yp-gray-200 bg-yp-red-soft px-3.5 py-2 text-left transition-colors hover:border-yp-red"
        >
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-yp-red text-white">
            <Wallet className="h-4 w-4" />
          </span>
          <span>
            <span className="block text-[10px] font-bold text-yp-gray-500">
              {benefit.cycleLabel} 남은 지원금
            </span>
            <span className="mt-0.5 block text-sm font-extrabold leading-none text-yp-red">
              개인 {benefit.personalBook.remainingAmount.toLocaleString()}원 · 추천{' '}
              {benefit.recommendedBook.limitCount - benefit.recommendedBook.usedCount}권
            </span>
          </span>
        </button>
      </div>
      ...
      <div className="border-t border-yp-gray-200">
        <div className="scrollbar-none mx-auto flex max-w-7xl items-center justify-between overflow-x-auto px-4 text-sm font-medium text-yp-ink">
          <div className="flex items-center whitespace-nowrap">
            <button
              type="button"
              onClick={() => onPendingNav('전체 카테고리')}
              className="mr-3 flex cursor-pointer items-center gap-1.5 border-r border-yp-gray-200 px-3 py-3 font-bold hover:text-yp-red"
            >
          ...
          <div className="flex items-center gap-3 whitespace-nowrap pl-4 text-xs text-yp-gray-500">
            ...
            <span className="cursor-pointer font-bold text-yp-ink hover:text-yp-red">
              트렌드 코리아 2027
            </span>
          </div>
        </div>
