...
export const RecommendedBooksPage: React.FC<RecommendedBooksPageProps> = ({ cart, onAddToCart, onBuyNow, onRemoveFromCart, onOpenBook }) => {
  ...
  return (
    <div className='flex-1'>
      ...
      <header className='flex flex-col items-center gap-[22px] bg-gradient-to-br from-[#8d1b12] via-yp-red to-[#c53a2b] px-6 pb-[72px] pt-16 text-center text-white'>
        ...
        <h1 className='text-[52px] font-bold leading-none tracking-[-1.5px]'>{PROGRAM.monthNumber}월 추천도서</h1>
      ...
      <main className='mx-auto flex w-full max-w-[1000px] flex-col items-center gap-12 px-4 pb-[90px] pt-[72px]'>
        ...
        <div className='flex w-full flex-col gap-12 md:flex-row'>
          <div className='relative shrink-0 self-center md:self-start'>
            {book.recommender && <span className='absolute -left-3.5 -top-3.5 z-10 -rotate-[8deg] rounded px-4 py-5 text-xl font-bold text-white bg-yp-red'>{book.recommender.type} 픽</span>}
            <button type='button' aria-label={`${book.title} 상세 보기`} onClick={() => onOpenBook(book.id)} className='cursor-pointer'>
              <BookCover book={book} className='h-[373px] w-[280px]' titleClassName='text-[28px] leading-[1.35]' />
            </button>
          </div>
          ...
          <div className='flex min-w-0 flex-1 flex-col gap-4 pt-2'>
            <button type='button' onClick={() => onOpenBook(book.id)} className='cursor-pointer text-left'>
              <h3 className='break-keep text-[34px] font-bold leading-tight tracking-[-1px] text-yp-ink hover:underline hover:underline-offset-4'>{book.title}</h3>
            </button>
            ...
            {book.curationReason && (
              <div className='rounded bg-yp-gray-50 px-[26px] py-6'>
                <p className='break-keep text-[16px] leading-[1.85] text-yp-gray-800'>{book.curationReason}</p>
              </div>
            )}

            <div className='grid grid-cols-[1fr_1.8fr] gap-3'>
              {inCart ? (
                <button type='button' onClick={() => onRemoveFromCart(book.id)} className='flex h-16 cursor-pointer items-center justify-center gap-1.5 rounded-lg border-2 border-yp-red bg-white text-[18px] font-bold text-yp-red transition-colors hover:bg-yp-red-soft'>
                  <Check className='h-5 w-5' />
                  담음 · 빼기
                </button>
              ) : (
                <button type='button' onClick={() => onAddToCart(book.id)} className='h-16 cursor-pointer rounded-lg border border-yp-gray-200 bg-white text-[18px] font-bold text-yp-ink transition-colors hover:bg-yp-gray-50'>
                  장바구니 담기
                </button>
              )}
              <button type='button' onClick={() => onBuyNow(book.id)} className='h-16 cursor-pointer rounded-lg bg-yp-red text-[18px] font-bold text-white transition-opacity hover:opacity-92'>
                추천도서 구매하기
              </button>
            </div>
          </div>
        ...
        <div className='flex w-full flex-col gap-5'>
          <div className='flex items-center gap-4'>
            ...
            <div ref={trackRef} onScroll={syncScroll} className='flex flex-1 gap-3.5 overflow-x-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'>
              {RECOMMENDED_BOOKS.map((item) => (
                <button
                  ...
                  className={`relative shrink-0 cursor-pointer rounded transition-opacity ${item.id === book.id ? 'ring-[3px] ring-yp-red' : 'opacity-55 hover:opacity-80'}`}
                >
                  <BookCover book={item} className='h-32 w-24' titleClassName='text-[12px] leading-[1.3]' />
                  {cartIds.has(item.id) && (
          ...
          <div className='relative h-[3px] w-full rounded-full bg-yp-gray-200'>
            <div className='absolute top-0 h-[3px] rounded-full bg-yp-red transition-all' style={{ width: `${progressWidth}%`, left: `${progressLeft}%` }} />
          </div>
        </div>
