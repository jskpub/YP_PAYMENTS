...
export const CartView: React.FC<CartViewProps> = ({
  ...
  const recommendedDiscount = hasRecommendedInCart
    ? (selectedBooks.find((b) => b.supportType === 'recommended')?.salePrice || 0)
    : 0;

  // 2) 개인도서 지원금: 개인도서나 일반도서 중 50% 지원, 최대 잔여한도 5,000원까지 차감
  const personalEligibleTotal = selectedBooks
    .filter((b) => b.supportType !== 'recommended')
    .reduce((acc, item) => acc + (item.salePrice * (item.quantity || 1)), 0);

  const personalBenefitDiscount = Math.min(
    Math.floor(personalEligibleTotal * 0.5),
      ...
      <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-lg p-3.5 flex items-start gap-2.5 text-xs text-blue-900 shadow-xs">
        ...
        <div className="leading-relaxed">
          ...
          <p className="text-blue-800 mt-0.5">
            현재 담긴 도서 중 <strong className="text-blue-950">[추천 도서 100% 무료 지원]</strong> 또는{' '}
            <strong className="text-blue-950">
              [개인 도서 반기 누적 한도 잔여 {benefitState.personalBook.remainingAmount.toLocaleString()}원(50%)]
            </strong>
            을 다음 주문/결제 단계에서 자유롭게 적용하여 결제금액을 할인받으실 수 있습니다.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {cartBooks.length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-xl p-12 text-center text-gray-400 text-sm">
              ...
              <button 
                ...
                className="text-[#D7001E] underline font-bold ml-2 cursor-pointer"
              >
                위시리스트 보러가기
              </button>
            </div>
            ...
            <div className="space-y-3">
              {cartBooks.map((book) => {
                ...
                return (
                  <div
                    key={book.id}
                    className={`bg-white border rounded-xl p-4 transition-all hover:shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                      isSelected ? 'border-red-200 shadow-xs' : 'border-gray-200'
                    }`}
                  >
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      {/* 도서 표지 */}
                      <div
                        className="w-14 h-19 rounded shrink-0 shadow-xs flex flex-col justify-between p-1.5 text-white font-bold text-center select-none"
                        style={{ backgroundColor: book.coverColor }}
                      >
                        <span className="text-[8px] uppercase tracking-tighter opacity-80 font-mono">YP</span>
                        <span className="text-[9px] leading-tight whitespace-pre-line my-auto">
                    ...
                    <div className="flex flex-row sm:flex-col items-end sm:items-end justify-between sm:justify-center w-full sm:w-36 shrink-0 gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                      ...
                      <div className="flex items-center border border-gray-300 rounded bg-white overflow-hidden text-xs">
                        ...
                        <span className="px-3 py-1 font-bold text-gray-800 min-w-[28px] text-center">
                          {quantity}
                        </span>
                        <button
                          type="button"
                      ...
                      <div className="flex items-center gap-2 w-full">
                        <button
                          ...
                          className="flex-1 py-1.5 px-2 text-xs font-bold text-white bg-[#D7001E] rounded hover:bg-red-700 transition-colors cursor-pointer text-center"
                        >
                          주문하기
                        </button>
                        <button
                          ...
                          className="py-1.5 px-2 text-xs text-gray-500 hover:text-red-600 border border-gray-300 rounded hover:bg-gray-50 cursor-pointer"
                        >
                          삭제
                        </button>
                      </div>
          ...
          <div className="flex items-center justify-between text-xs text-gray-600 pt-2">
            ...
            <button
              type="button"
              onClick={() => setActiveTab('wishlist')}
              className="text-[#D7001E] font-semibold hover:underline cursor-pointer flex items-center gap-1"
            >
              <span>쇼핑 계속하기</span>
            ...
            <div className="space-y-2.5 text-xs text-gray-600">
              ...
              <div className="border-t border-gray-200 pt-3 mt-3 flex justify-between items-baseline">
                ...
                <div className="text-right">
                  <span className="text-2xl font-black text-[#D7001E]">
