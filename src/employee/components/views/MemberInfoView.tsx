...
export const MemberInfoView: React.FC<MemberInfoViewProps> = ({ benefitState, setBenefitState, setActiveTab }) => {
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-baseline justify-between border-b border-gray-200 pb-3'>
        <div>
          ...
          <p className='text-xs text-gray-500 mt-1'>회원님의 소중한 개인정보를 안전하게 관리합니다. 최신 정보로 업데이트해 주세요.</p>
        </div>
      ...
      <form onSubmit={handleSave} className='bg-white border border-gray-200 rounded-xl p-6 shadow-xs space-y-6 text-xs text-gray-800'>
          ...
          <div className='flex items-center gap-4'>
            ...
            <div className='space-y-1'>
              <div className='flex items-center gap-2'>
                <button type='button' onClick={() => alert('프로필 이미지 파일 선택')} className='px-3 py-1 bg-white border border-gray-300 rounded text-xs text-gray-700 hover:bg-gray-50 cursor-pointer'>
                  이미지 변경
                </button>
                <button type='button' className='px-3 py-1 bg-white border border-gray-200 rounded text-xs text-gray-400 hover:text-gray-600 cursor-pointer'>
                  삭제
                </button>
              </div>
        ...
        <div className='flex flex-col sm:flex-row sm:items-start gap-4 pb-5 border-b border-gray-100'>
          ...
          <div className='flex-1 space-y-1'>
            <div className='flex items-center gap-2'>
              <input type='text' disabled value={benefitState.user.employeeId} className='w-56 p-2 bg-gray-100 border border-gray-300 rounded  font-bold text-gray-700' />
        ...
        <div className='flex flex-col sm:flex-row sm:items-center gap-4 pb-5 border-b border-gray-100'>
          ...
          <div className='flex-1'>
            <input type='text' required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className='w-56 p-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#D7001E]' />
          </div>
        </div>
        ...
        <div className='flex flex-col sm:flex-row sm:items-start gap-4 pb-5 border-b border-gray-100'>
          ...
          <div className='flex-1 space-y-2'>
            <div className='flex items-center gap-2'>
              <input type='text' required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className='w-56 p-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#D7001E]' />
              <button type='button' onClick={() => alert('휴대전화 번호 변경 인증 창입니다.')} className='px-2.5 py-1 text-xs border border-gray-300 rounded text-gray-700 hover:bg-gray-50 cursor-pointer'>
                번호 변경 / 재인증
              </button>
            </div>
            <label className='flex items-center gap-2 text-gray-600 cursor-pointer'>
        ...
        <div className='flex flex-col sm:flex-row sm:items-start gap-4 pb-5 border-b border-gray-100'>
          ...
          <div className='flex-1 space-y-2'>
            <div className='flex items-center gap-2 flex-wrap'>
              <input type='text' value={formData.birthYear} onChange={(e) => setFormData({ ...formData, birthYear: e.target.value })} className='w-16 p-1.5 border border-gray-300 rounded text-center' />
              <span>년</span>
              <input type='text' value={formData.birthMonth} onChange={(e) => setFormData({ ...formData, birthMonth: e.target.value })} className='w-12 p-1.5 border border-gray-300 rounded text-center' />
              <span>월</span>
              <input type='text' value={formData.birthDay} onChange={(e) => setFormData({ ...formData, birthDay: e.target.value })} className='w-12 p-1.5 border border-gray-300 rounded text-center' />
              <span>일</span>

              <div className='flex items-center gap-3 ml-4'>
                <label className='flex items-center gap-1 cursor-pointer'>
                  <input type='radio' name='calendar' checked={formData.calendarType === 'solar'} onChange={() => setFormData({ ...formData, calendarType: 'solar' })} className='accent-[#D7001E]' />
                  <span>양력</span>
                </label>
                <label className='flex items-center gap-1 cursor-pointer'>
                  <input type='radio' name='calendar' checked={formData.calendarType === 'lunar'} onChange={() => setFormData({ ...formData, calendarType: 'lunar' })} className='accent-[#D7001E]' />
                  <span>음력</span>
                </label>
              </div>
              ...
              <div className='flex items-center gap-3 ml-4 border-l border-gray-200 pl-4'>
                <label className='flex items-center gap-1 cursor-pointer'>
                  <input type='radio' name='gender' checked={formData.gender === 'male'} onChange={() => setFormData({ ...formData, gender: 'male' })} className='accent-[#D7001E]' />
                  <span>남성</span>
                </label>
                <label className='flex items-center gap-1 cursor-pointer'>
                  <input type='radio' name='gender' checked={formData.gender === 'female'} onChange={() => setFormData({ ...formData, gender: 'female' })} className='accent-[#D7001E]' />
                  <span>여성</span>
                </label>
              </div>
            </div>
        ...
        <div className='flex flex-col sm:flex-row sm:items-start gap-4 pb-5 border-b border-gray-100'>
          ...
          <div className='flex-1 space-y-2 max-w-xl'>
            <div className='flex items-center gap-2'>
              <input type='text' defaultValue='06123' readOnly className='w-24 p-2 bg-gray-50 border border-gray-300 rounded ' />
              <button type='button' onClick={() => alert('우편번호 검색 팝업입니다.')} className='px-3 py-2 bg-gray-800 text-white rounded text-xs hover:bg-gray-900 cursor-pointer'>
                우편번호 찾기
              </button>
            </div>
            <input type='text' required value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} className='w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#D7001E]' />
            <input type='text' value={formData.detailAddress} onChange={(e) => setFormData({ ...formData, detailAddress: e.target.value })} className='w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#D7001E]' />
            <label className='flex items-center gap-2 text-gray-600 cursor-pointer'>
        ...
        <div className='flex items-center justify-center gap-3 pt-3'>
          <button type='button' onClick={() => setActiveTab('benefit')} className='px-6 py-2.5 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 font-medium cursor-pointer'>
            취소
          </button>
          <button type='submit' className='px-8 py-2.5 bg-[#D7001E] text-white font-bold rounded-md hover:bg-red-700 transition-colors shadow-xs cursor-pointer'>
            회원정보 저장하기
          </button>
        </div>
