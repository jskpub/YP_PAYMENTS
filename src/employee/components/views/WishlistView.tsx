...
export const WishlistView: React.FC<WishlistViewProps> = ({ books, setBooks, onAddToCart, onBuyNow, setActiveTab, benefitState }) => {
    ...
    <div className='space-y-6'>
      ...
      <div className='border-b border-gray-200 pb-3 flex flex-col md:flex-row md:items-center justify-between gap-2'>
        <div>
          <h1 className='text-2xl font-black text-gray-900 tracking-tight flex items-center gap-2'>
            ...
            <span className='text-xs font-normal text-gray-500'>관심 도서로 등록하신 상품 목록입니다. 지원금 혜택을 확인하고 구매해 보세요.</span>
          </h1>
        </div>
      ...
      <div className='flex items-center justify-between border-b border-gray-200 text-xs'>
        <div className='flex items-center gap-2 font-medium'>
          <button type='button' onClick={() => setSubTab('all')} className={`py-2 px-3 border-b-2 font-bold cursor-pointer transition-colors ${subTab === 'all' ? 'border-[#D7001E] text-[#D7001E]' : 'border-transparent text-gray-600 hover:text-gray-900'}`}>
            전체 ({books.length})
          </button>
          <button type='button' onClick={() => setSubTab('benefit')} className={`py-2 px-3 border-b-2 font-bold cursor-pointer transition-colors flex items-center gap-1 ${subTab === 'benefit' ? 'border-[#D7001E] text-[#D7001E]' : 'border-transparent text-gray-600 hover:text-[#D7001E]'}`}>
            <span className='w-1.5 h-1.5 rounded-full bg-[#D7001E]'></span>
            <span>임직원 추천/지원도서 (3)</span>
          </button>
          <button type='button' onClick={() => setSubTab('normal')} className={`py-2 px-3 border-b-2 font-bold cursor-pointer transition-colors ${subTab === 'normal' ? 'border-[#D7001E] text-[#D7001E]' : 'border-transparent text-gray-600 hover:text-gray-900'}`}>
            일반 개인도서 (2)
          </button>
        </div>

        <button type='button' onClick={() => alert('새 폴더 추가 기능입니다.')} className='text-gray-600 hover:text-[#D7001E] flex items-center gap-1 font-medium py-1 px-2 border border-gray-200 rounded hover:bg-gray-50 cursor-pointer'>
          <Plus size={12} />
          <span>폴더추가</span>
        </button>
      </div>
      ...
      <div className='space-y-3'>
        {filteredBooks.length === 0 ? (
          <div className='bg-white border border-gray-200 rounded-lg p-12 text-center text-gray-400 text-sm'>등록된 위시리스트 도서가 없습니다.</div>
        ) : (
          filteredBooks.map((book) => {
            const isSelected = selectedBookIds.includes(book.id);

            return (
              <div key={book.id} className={`bg-white border rounded-xl p-4 transition-all hover:shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4 ${isSelected ? 'border-red-200' : 'border-gray-200'}`}>
                <div className='flex items-start gap-4 flex-1'>
                  <input type='checkbox' checked={isSelected} onChange={() => handleToggleSelectBook(book.id)} className='accent-[#D7001E] w-4 h-4 rounded mt-1.5 cursor-pointer' />

                  {/* 도서 가상 커버 */}
                  <div className='w-16 h-22 rounded shrink-0 shadow-xs flex flex-col justify-between p-1.5 text-white font-bold text-center select-none' style={{ backgroundColor: book.coverColor }}>
                    <span className='text-[9px] uppercase tracking-tighter opacity-80 '>YP BOOKS</span>
                    <span className='text-[10px] leading-tight whitespace-pre-line my-auto'>{book.coverLabel || book.title.replace(/[『』]/g, '')}</span>
                  ...
                  <div className='space-y-1 flex-1'>
                    ...
                    <div className='flex items-center gap-3 text-xs pt-1 flex-wrap'>
                      <span className='text-gray-400 line-through'>정가 {book.originalPrice.toLocaleString()}원</span>
                      <span className='font-bold text-gray-900'>{book.salePrice.toLocaleString()}원</span>
                      <span className='text-[#D7001E] font-bold'>({book.discountRate}%↓)</span>
                ...
                <div className='w-full md:w-40 flex flex-row md:flex-col items-center gap-1.5 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-gray-100'>
                  ...
                  <button type='button' onClick={() => onBuyNow(book)} className='w-full py-2 px-3 text-xs font-bold text-white bg-[#D7001E] rounded hover:bg-red-700 transition-colors cursor-pointer shadow-xs'>
                    바로구매
                  </button>
