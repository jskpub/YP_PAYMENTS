...
export const OrdersView: React.FC<OrdersViewProps> = ({
  ...
  const handleCancelOrder = (orderNumber: string) => {
    ...
    const confirmMsg = `주문번호 [${orderNumber}]을(를) 취소하시겠습니까?\n\n[지원금 자동 복원 안내]\n${
      ...
      restoredPersonalAmount > 0
        ? `• 개인 도서 지원금: ${restoredPersonalAmount.toLocaleString()}원 잔여 한도로 즉시 환원\n`
        : ''
    ...
    setBenefitState((prev) => ({
      ...
      personalBook: {
        ...
        remainingAmount: Math.min(
          prev.personalBook.totalLimit,
          prev.personalBook.remainingAmount + restoredPersonalAmount
        ),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-200 pb-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            최근 주문하신 상품의 배송 상태 및 복합 지원금 결제 내역을 확인하실 수 있습니다.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-xs text-[#D7001E] bg-red-50 hover:bg-red-100 px-2.5 py-1 rounded border border-red-200 font-bold flex items-center gap-1 cursor-pointer"
          >
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs space-y-3">
        <div className="grid grid-cols-5 gap-2 text-center text-xs">
          ...
          <div className="p-2 rounded bg-red-50/70 border border-red-100">
            ...
            <p className="text-xl font-black text-[#D7001E]">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between text-[11px] text-gray-400 pt-2 border-t border-gray-100 gap-2">
          ...
          <div className="flex items-center gap-3 text-gray-600">
            <span>취소 <strong>{orders.filter((o) => o.status === '주문취소').length}</strong></span>
            <span>|</span>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 text-xs">
          ...
          {['1주일', '1개월', '3개월', '6개월'].map((p) => (
            <button
              ...
              className={`px-2.5 py-1 rounded border text-xs cursor-pointer ${
                selectedPeriod === p ? 'bg-gray-800 text-white border-gray-800 font-bold' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-100'
              }`}
            >
              {p}
            </button>
        ...
        <div className="relative w-full md:w-64">
          <input
            type="text"
            placeholder="도서명 또는 주문번호 검색"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-3 pr-8 py-1.5 border border-gray-300 rounded text-xs bg-white focus:outline-none focus:ring-1 focus:ring-[#D7001E]"
          />
            ...
            return (
              <div 
                ...
                className={`bg-white border rounded-xl overflow-hidden shadow-xs transition-all ${
                  isCancelled ? 'border-gray-300 opacity-80' : 'border-gray-200'
                }`}
              >
                {/* 주문 헤더 */}
                <div className="bg-[#F8F9FA] px-4 py-2.5 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between text-xs gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-gray-600">주문번호 {order.orderNumber}</span>
                    {isCancelled && (
                  ...
                  <button
                    type="button"
                    onClick={() => onOpenOrderDetail(order.orderNumber)}
                    className="text-gray-600 hover:text-[#D7001E] font-medium flex items-center gap-0.5 cursor-pointer self-end sm:self-auto"
                  >
                    <span>주문상세보기</span>
                  ...
                  {order.items.map((item) => (
                    <div key={item.bookId} className="pt-3 first:pt-0 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-3.5 flex-1">
                        {/* 도서 표지 */}
                        <div
                          className="w-13 h-18 rounded shrink-0 shadow-xs flex flex-col justify-between p-1 text-white font-bold text-center select-none"
                          style={{ backgroundColor: item.coverColor }}
                        >
                          <span className="text-[7px] uppercase tracking-tighter opacity-80 font-mono">YP</span>
                          <span className="text-[8px] leading-tight whitespace-pre-line my-auto">
                            {item.coverLabel || item.title.replace(/[『』]/g, '')}
                          </span>
                          ...
                          <div className="text-xs text-gray-700 flex items-center gap-2 flex-wrap pt-0.5">
                            ...
                            {item.supportType === 'recommended' ? (
                              <span className="text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                                🌟 추천도서 100% 전액지원: {item.salePrice.toLocaleString()}원 전액차감 (본인부담 0원)
                              </span>
                            ) : (
                              <span>
                      ...
                      <div className="w-full sm:w-36 shrink-0 flex flex-col items-start sm:items-end gap-1.5 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                        <div className="text-right">
                          ...
                          {!isCancelled && order.statusDescription && item.status === '상품준비중' && (
                        ...
                        <div className="flex items-center gap-1.5 w-full sm:justify-end">
                          {!isCancelled && (
                            <button
                              type="button"
                              onClick={() => alert(`배송조회: ${item.title}\n상태: ${item.status}\n배송지: ${order.shippingAddress}`)}
                              className="py-1 px-2 text-[11px] border border-gray-300 rounded hover:bg-gray-50 text-gray-700 cursor-pointer"
                            >
                              배송조회
                            </button>
                          )}
                          ...
                          {isCancelled ? (
                            ...
                            <button
                              type="button"
                              onClick={() => alert('리뷰 작성 시 300P 적립!')}
                              className="py-1 px-2 text-[11px] font-bold text-white bg-[#D7001E] rounded hover:bg-red-700 cursor-pointer"
                            >
                              리뷰작성 +300P
                            </button>
                          ) : (
                            <>
                              ...
                              <button
                                ...
                                className="py-1 px-2 text-[11px] text-red-600 font-bold border border-red-200 bg-red-50/50 rounded hover:bg-red-100 cursor-pointer transition-colors"
                              >
                                주문취소
                              </button>
                            </>
                ...
                <div className="bg-[#FAFBFD] px-4 py-3 border-t border-gray-200 flex flex-col md:flex-row md:items-center justify-between text-xs text-gray-700 gap-2">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span>결제수단: <strong>{order.paymentMethod}</strong></span>
                    <span className="text-gray-300">|</span>
                    <span>총 주문금액: <strong>{order.totalPrice.toLocaleString()}원</strong></span>
                    <span className="text-gray-300">|</span>
                    <span className="text-[#D7001E] font-medium">
                      임직원 도서 지원금: <strong>{order.totalBenefitAmount.toLocaleString()}원</strong>
                      {order.items.some(i => i.supportType === 'recommended') && (
                        <span className="text-emerald-700 text-[10px] ml-1 font-bold">
                          (추천 100% + 개인 50% 복합적용)
                        </span>
                      )}
                    </span>
