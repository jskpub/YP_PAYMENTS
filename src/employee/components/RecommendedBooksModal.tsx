...
export const RecommendedBooksModal: React.FC<RecommendedBooksModalProps> = ({
  isOpen,
  onClose,
  onSelectBook,
}) => {
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100">
                100% 임직원 전액 지원 대상 (정가 전액 무료 지원, 본인부담금 0원)
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50 space-y-2 text-xs">
          <div className="relative">
            <input
              ...
              className="w-full pl-3 pr-8 py-2 border border-gray-300 rounded-lg bg-white text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
            />
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            {categories.map((cat) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-medium cursor-pointer whitespace-nowrap ${
        ...
        <div className="flex-1 overflow-y-auto p-4 divide-y divide-gray-100 text-xs">
          {filtered.map((book) => (
            <div
              key={book.id}
              className="py-3 flex items-center justify-between gap-4 hover:bg-gray-50/80 px-2 rounded-lg transition-colors"
            >
              <div>
        ...
        <div className="p-3 bg-gray-50 border-t border-gray-200 text-center text-xs text-gray-500">
          사내 권장도서는 반기당 1권에 한해 전액 무료(본인부담금 0원)로 지원됩니다.
        </div>
      </div>
    </div>
  );
