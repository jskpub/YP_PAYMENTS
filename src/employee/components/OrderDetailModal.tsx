      ...
      <div className='bg-white rounded-2xl max-w-xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 text-xs'>
        ...
        <div className='flex-1 overflow-y-auto p-5 space-y-4'>
          ...
          <div className='space-y-2'>
            <h4 className='font-bold text-gray-900 border-b border-gray-100 pb-1.5'>주문 상품 ({order.items.length}종)</h4>
            <div className='divide-y divide-gray-100'>
              {order.items.map((item) => (
                <div key={item.bookId} className='py-2.5 flex justify-between items-start gap-3'>
                  <div>
                    <div className='flex items-center gap-1.5 mb-0.5'>
                      <span className='bg-gray-100 text-gray-700 text-[10px] font-bold px-1.5 py-0.2 rounded'>{item.category}</span>
        ...
        <div className='p-3 bg-gray-50 border-t border-gray-200 flex justify-end'>
          <button type='button' onClick={onClose} className='px-4 py-1.5 bg-gray-800 text-white rounded font-medium hover:bg-gray-900 cursor-pointer'>
            닫기
          </button>
        </div>
      </div>
