      ...
      <div className="relative w-full max-w-2xl bg-white rounded-lg shadow-2xl overflow-hidden my-auto animate-in fade-in duration-150">
        ...
        <div className="p-6 text-sm text-[#181718] space-y-4">
          <div className="flex justify-between items-start border-b border-[#dadada] pb-4">
            ...
            <div className="text-right text-xs">
              <p className="font-semibold text-sm">신청 임직원: 김지선</p>
          ...
          <table className="w-full text-xs text-left border border-[#cbd2d4]">
              ...
              {selectedItems.map((item, idx) => (
                <tr key={idx}>
                  <td className="p-2.5">
                    ...
                    <div className="text-[#80888a]">{item.book.author} | {item.book.publisher}</div>
                  </td>
                  <td className="p-2.5 text-center font-medium">
                    <div className={item.book.bookType === 'recommended' ? 'text-[#df0000] font-bold' : 'text-[#1f976b] font-bold'}>
                      {item.book.bookType === 'recommended' ? '추천 (100%)' : '개인 (50%)'}
                    </div>
                    <div className="text-[11px] text-[#555a5c]">
                      {item.format === 'ebook' ? '전자책' : '종이책'}
                    </div>
                  </td>
                  <td className="p-2.5 text-center">{item.quantity}</td>
                  <td className="p-2.5 text-right text-[#80888a]">
                    {(item.book.listPrice * item.quantity).toLocaleString()}원
                  </td>
                  <td className="p-2.5 text-right font-semibold">
                    {item.itemSellingPrice.toLocaleString()}원
                  </td>
                  <td className="p-2.5 text-right text-[#1f976b] font-medium">
                    -{item.itemCompanySubsidy.toLocaleString()}원
                  </td>
                  <td className="p-2.5 text-right text-[#df0000] font-bold">
            ...
            <tfoot className="bg-[#fffafa] border-t-2 border-[#cbd2d4] font-bold">
              <tr>
                <td colSpan={4} className="p-3 text-right">합계 견적 금액:</td>
          ...
          <div className="text-xs text-[#80888a] bg-[#f6f6f6] p-3 rounded leading-relaxed">
            * 배송비: {cartStats.shippingFee.toLocaleString()}원 (30,000원 이상 주문 시 무료배송)<br />
            * 최종 실결제 예상금액: <strong className="text-[#df0000] text-sm">{cartStats.finalPaymentAmount.toLocaleString()}원</strong>
          </div>
        </div>
        ...
        <div className="bg-[#f6f6f6] px-5 py-3.5 flex justify-end gap-2 border-t border-[#dadada]">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded bg-white border border-[#cbd2d4] text-xs font-semibold text-[#181718]"
          >
          ...
          <button
            onClick={() => setIsEstimateModalOpen(false)}
            className="px-5 py-2 rounded bg-[#df0000] text-white text-xs font-bold"
          >
            닫기
          </button>
        </div>
