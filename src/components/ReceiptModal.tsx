    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto'>
      <div className='relative w-full max-w-lg bg-white rounded-lg shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-150'>
        ...
        <div className='bg-[#181718] text-white px-5 py-4 flex items-center justify-between'>
          ...
          <button onClick={() => setIsReceiptModalOpen(false)} className='text-neutral-400 hover:text-white p-1' aria-label='닫기'>
            <X className='w-5 h-5' />
          </button>
        </div>
        ...
        <div className='p-6 text-sm text-[#181718] space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-2 text-xs bg-[#f6f6f6] p-3 rounded'>
            <div>
              <span className='text-[#80888a]'>주문자:</span> <span className='font-semibold'>{order.employeeName}</span> (임직원)
            </div>
            <div>
              <span className='text-[#80888a]'>소속사:</span> <span className='font-semibold'>(주)파트너스 B2B</span>
            </div>
            <div className='col-span-2'>
              <span className='text-[#80888a]'>배송지:</span> {order.deliveryAddress.roadAddress} {order.deliveryAddress.detailAddress}
            </div>
          </div>
          ...
          <div className='space-y-1.5 text-xs bg-[#fffafa] border border-[#f9cdcd] p-3.5 rounded'>
            ...
            <div className='text-[12px] text-[#80888a] text-right pt-1'>결제수단: {order.paymentMethod}</div>
          </div>

          {/* Notice */}
          <div className='text-[12px] text-[#80888a] text-center leading-relaxed'>* 본 영수증은 회사 지원금 및 직원 결제가 하나의 주문번호로 승인된 복합결제 증빙용 영수증입니다.</div>
        </div>

        {/* Footer */}
        <div className='bg-[#f6f6f6] px-5 py-3.5 flex justify-end gap-2 border-t border-[#dadada]'>
          <button onClick={handlePrint} className='flex items-center gap-1.5 px-4 py-2 rounded bg-white border border-[#cbd2d4] hover:bg-[#edf0f1] text-xs font-semibold text-[#181718]'>
            <Printer className='w-3.5 h-3.5' />
            인쇄하기
          </button>
          <button onClick={() => setIsReceiptModalOpen(false)} className='px-5 py-2 rounded bg-[#df0000] text-white text-xs font-bold hover:bg-[#ea2e2e]'>
            확인
          </button>
        </div>
      </div>
