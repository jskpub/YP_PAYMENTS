  ...
  return (
    <div className="flex items-center text-[15px] sm:text-[16px] select-none">
      {steps.map((step, idx) => {
        ...
        const isPast =
          (currentStep === 'payment' && (step.key === 'cart' || step.key === 'gift')) ||
          (currentStep === 'gift' && step.key === 'cart') ||
          ...
          <React.Fragment key={step.key}>
            <button
              ...
              className={`flex items-center gap-1.5 transition-colors ${
                isActive
                  ? 'font-bold text-[#181718]'
                  : isPast
                  ? 'text-[#595959] hover:text-[#181718]'
                  : 'text-[#9c9c9c] hover:text-[#595959]'
              }`}
            >
              {isActive && (
                <span className="w-5 h-5 rounded-full bg-[#181718] text-white flex items-center justify-center text-xs font-bold">
                  {step.number}
                </span>
              )}
            ...
            {idx < steps.length - 1 && (
              <ChevronRight className="w-4 h-4 mx-2 sm:mx-3 text-[#cbd2d4]" />
            )}
          </React.Fragment>
        );
      })}
