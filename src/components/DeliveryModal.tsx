        ...
        <div className="p-5 overflow-y-auto flex-1 text-[#3d3c3f] text-sm">
          ...
          {addressModalTab === 'list' && (
            <div className="space-y-3">
              <div className="text-xs text-[#80888a] mb-2">
                등록된 배송지 목록 중 배송받으실 주소를 선택해주세요.
              </div>
              {addresses.map((addr) => {
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {addr.isDefault && (
                          <span className="text-[11px] bg-[#df0000] text-white px-2 py-0.5 rounded font-medium">
                            기본배송지
                          </span>
                        )}
                      </div>
              ...
              <div className="pt-3 text-center">
                <button
                  type="button"
                  onClick={() => setAddressModalTab('new')}
                  className="text-xs font-semibold text-[#df0000] hover:underline"
                >
                  + 새로운 배송지 등록하기
                </button>
              </div>
          ...
          {addressModalTab === 'recent' && (
            <div className="space-y-3">
              <div className="text-xs text-[#80888a] mb-2">
                최근 주문 시 사용하셨던 배송지 내역입니다.
              </div>
              {addresses.slice(0, 3).map((addr) => (
                <div
                  key={addr.id}
                  className="p-4 rounded border border-[#cbd2d4] hover:border-[#80888a] bg-white flex items-start justify-between"
                >
                  <div>
                    <span className="font-bold text-sm text-[#181718]">{addr.recipient} ({addr.title})</span>
                  ...
                  <button
                    type="button"
                    onClick={() => selectExistingAddress(addr)}
                    className="text-xs px-3 py-1.5 rounded font-semibold border bg-white text-[#df0000] border-[#df0000] hover:bg-[#ffebeb]"
                  >
                    이 주소로 선택
                  </button>
                </div>
            ...
            <form onSubmit={handleSubmitNewAddress} className="space-y-3.5">
              ...
              <div className="flex items-center gap-6 pb-1">
                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium">
                  <input
                    type="radio"
                    name="deliveryMethod"
                    checked={deliveryType === 'domestic'}
                    onChange={() => setDeliveryType('domestic')}
                ...
                <label className="flex items-center gap-2 cursor-pointer text-sm text-[#80888a]">
                  <input
                    type="radio"
                    name="deliveryMethod"
                    checked={deliveryType === 'overseas'}
                    onChange={() => setDeliveryType('overseas')}
                    className="w-4 h-4 accent-[#df0000]"
                  />
              ...
              <div className="grid grid-cols-[90px_1fr] sm:grid-cols-[100px_1fr] items-center gap-2">
                ...
                <input
                  ...
                  placeholder="배송지명을 입력하세요 (예: 자택, 회사)"
                  className="w-full h-10 px-3 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm placeholder-[#9da6a8]"
                />
              </div>
              ...
              <div className="grid grid-cols-[90px_1fr] sm:grid-cols-[100px_1fr] items-center gap-2">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 h-10 px-3 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm placeholder-[#9da6a8]"
                  />
              ...
              <div className="grid grid-cols-[90px_1fr] sm:grid-cols-[100px_1fr] items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="w-20 h-10 px-2 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm bg-white"
                  >
                  ...
                  <input
                    ...
                    className="w-20 h-10 px-2 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm text-center"
                  />
                  <span className="text-[#9da6a8]">-</span>
                  <input
                    type="text"
                    maxLength={4}
                    value={phoneEnd}
                    onChange={(e) => setPhoneEnd(e.target.value.replace(/[^0-9]/g, ''))}
                    className="w-20 h-10 px-2 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm text-center"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-[90px_1fr] sm:grid-cols-[100px_1fr] items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <select
                    value={phone2Prefix}
                    onChange={(e) => setPhone2Prefix(e.target.value)}
                    className="w-20 h-10 px-2 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm bg-white text-[#80888a]"
                  >
                  ...
                  <input
                    ...
                    maxLength={4}
                    value={phone2Mid}
                    onChange={(e) => setPhone2Mid(e.target.value.replace(/[^0-9]/g, ''))}
                    className="w-20 h-10 px-2 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm text-center"
                  />
                  <span className="text-[#9da6a8]">-</span>
                  <input
                    ...
                    maxLength={4}
                    value={phone2End}
                    onChange={(e) => setPhone2End(e.target.value.replace(/[^0-9]/g, ''))}
                    className="w-20 h-10 px-2 border border-[#cbd2d4] rounded focus:border-[#df0000] focus:outline-none text-sm text-center"
                  />
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      value={postalCode}
                      placeholder="우편번호"
                      className="w-28 h-10 px-3 border border-[#cbd2d4] rounded bg-[#f6f6f6] text-sm text-[#181718]"
                    />
                    <button
                      type="button"
                      onClick={() => setShowAddressPicker(true)}
                      className="h-10 px-4 border border-[#cbd2d4] rounded bg-white hover:bg-[#f6f6f6] text-sm font-semibold text-[#181718] transition-colors"
                    >
                      주소 검색
                    </button>
                  </div>
                  ...
                  <input
                    ...
                    value={roadAddress}
                    placeholder="도로명"
                    className="w-full h-10 px-3 border border-[#cbd2d4] rounded bg-[#f6f6f6] text-sm text-[#181718]"
                  />
                  <input
                    type="text"
                    readOnly
              ...
              <div className="bg-[#fffafa] border border-[#f9cdcd] rounded p-3.5 text-xs space-y-1 text-[#555a5c] leading-relaxed">
                <p className="text-[#df0000]">
                  • 당일배송 주문시는 반드시 주소를 재입력해주시기 바랍니다.
                </p>
                <p>
                  ...
                  <span className="text-[#df0000] font-semibold">1:1 상담</span>으로 반드시 연락주시기 바랍니다.
                </p>
                <p>
              ...
              <div className="flex items-center justify-center gap-3 pt-3 border-t border-[#edf0f1]">
                <button
                  type="button"
                  onClick={() => setIsAddressModalOpen(false)}
                  className="min-w-[120px] h-10 px-6 rounded border border-[#cbd2d4] bg-white text-sm font-semibold text-[#555a5c] hover:bg-[#f6f6f6] transition-colors"
                >
                  취소
                </button>
                <button
                  type="submit"
                  className="min-w-[120px] h-10 px-6 rounded bg-[#df0000] hover:bg-[#ea2e2e] text-sm font-bold text-white transition-colors shadow-sm"
                >
                  저장
                </button>
              </div>
                ...
                <div className="p-4 space-y-3">
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="도로명 또는 건물명 검색 (예: 종로, 강남대로, 서초구)"
                      className="flex-1 h-10 px-3 border border-[#cbd2d4] rounded text-sm focus:outline-none focus:border-[#df0000]"
                    />
                    <button
                      type="button"
                      className="px-4 bg-[#df0000] text-white rounded text-sm font-bold flex items-center gap-1"
                    >
                  ...
                  <div className="text-xs text-[#80888a]">
                    아래 검색 예시 주소를 클릭하여 바로 입력하실 수 있습니다.
                  </div>
                  <div className="max-h-60 overflow-y-auto divide-y divide-[#edf0f1] border border-[#cbd2d4] rounded">
                    {sampleAddresses
                      .filter(
                        (a) =>
                          !addressSearchTerm ||
                          a.road.includes(addressSearchTerm) ||
                          a.jibun.includes(addressSearchTerm)
                      )
                      .map((item, i) => (
                        <div
                          key={i}
